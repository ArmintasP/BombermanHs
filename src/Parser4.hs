...
runParser input = runParser' . fst $ (runState . runExceptT $ parse) (input, 0)

runParser' :: Either ParserError JsonLike -> Either String JsonLike
runParser' (Right x) = Right $ id x
runParser' (Left x) = Left $ show x
